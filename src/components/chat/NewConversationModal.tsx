...
export default function NewConversationModal({
  ...
  return (
    <div className="fixed inset-0 bg-bg/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in text-text-primary">
      <div className="bg-surface-elevated shadow-2xl rounded-xl w-full max-w-md overflow-hidden transform transition-all flex flex-col max-h-[80vh] border border-border">
        ...
        <div className="flex border-b border-border">
          <button 
            className={`flex-1 py-3 text-sm font-medium ${activeTab === 'personal' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary hover:bg-white/5'}`}
            onClick={() => setActiveTab('personal')}
          >
            Personal Chat
          </button>
          <button 
            className={`flex-1 py-3 text-sm font-medium ${activeTab === 'group' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary hover:bg-white/5'}`}
          ...
          {user?.role === 'teacher' && (
            <button 
              className={`flex-1 py-3 text-sm font-medium ${activeTab === 'teacher-group' ? 'text-primary border-b-2 border-primary' : 'text-text-secondary hover:bg-white/5'}`}
        ...
        <div className="p-6 overflow-y-auto flex-1 custom-scrollbar">
          ...
          {activeTab === 'personal' && (
            <div className="space-y-4">
              <input
                ...
                className="w-full bg-surface border-none rounded-lg px-4 py-3 text-text-body placeholder:text-text-secondary focus:ring-1 focus:ring-primary focus:outline-none"
                placeholder="Search contact by email..."
              />
              
              {isSearching && <div className="text-text-secondary text-sm">Searching...</div>}
              ...
              <div className="space-y-1 mt-4 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                {searchResults.map(user => (
                  <div key={user.id} className="flex items-center justify-between p-3 bg-surface hover:bg-border rounded-lg transition-colors group">
                    ...
                    <button 
                      ...
                      className="text-sm bg-primary hover:bg-primary-hover text-on-primary px-4 py-1.5 rounded font-medium transition-all opacity-0 group-hover:opacity-100"
                    >
                      Chat
                    </button>
                  </div>
          ...
          {activeTab === 'group' && (
            <form onSubmit={handleCreateGroup} className="space-y-4 flex flex-col h-full">
              {groupError && <div className="p-3 bg-error/10 text-error text-sm rounded">{groupError}</div>}
              <input
                type="text"
                value={groupName}
              ...
              <textarea
                ...
                className="w-full bg-surface border-none rounded-lg px-4 py-3 text-text-body placeholder:text-text-secondary focus:ring-1 focus:ring-primary focus:outline-none resize-none"
                placeholder="Description (Optional)"
                rows={2}
              />

              <div className="pt-2 border-t border-border">
                <p className="text-sm font-medium text-primary mb-2">Add Members</p>
                <input
                  type="text"
                  value={groupSearch}
                  onChange={handleGroupSearchChange}
                  className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-sm text-text-body placeholder:text-text-secondary focus:border-primary focus:outline-none"
                ...
                {groupResults.length === 0 && groupSearch.length > 0 && !isSearchingGroupUsers && (
                  <div className="mt-2 text-center text-sm text-text-secondary">No matching users found</div>
                )}
                ...
                {selectedGroupUsers.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4">
                    {selectedGroupUsers.map(user => (
                      <div key={user.id} className="flex items-center gap-1.5 bg-border px-2 py-1 rounded-md">
                        <div className="w-4 h-4 bg-primary rounded-full flex items-center justify-center text-[10px] text-white">
                          {user.avatar_url ? <img src={user.avatar_url} className="w-full h-full rounded-full object-cover" /> : user.name.charAt(0)}
                        </div>
                        <span className="text-xs text-text-primary">{user.name.split(' ')[0]}</span>
                        <button type="button" onClick={() => setSelectedGroupUsers(selectedGroupUsers.filter(u => u.id !== user.id))} className="text-text-secondary hover:text-error ml-1">
              ...
              <div className="mt-auto pt-6 flex justify-end">
                <button
                  ...
                  className="bg-primary hover:bg-primary-hover text-on-primary px-6 py-2.5 rounded font-bold transition-colors disabled:opacity-60"
                >
          ...
          {activeTab === 'teacher-group' && (
            <form onSubmit={handleCreateTeacherGroup} className="space-y-4 flex flex-col h-full">
              {teacherError && <div className="p-3 bg-error/10 text-error text-sm rounded">{teacherError}</div>}
              <input
                type="text"
              ...
              <div className="pt-2 border-t border-border">
                <p className="text-sm font-medium text-primary mb-2">Add Teachers</p>
                <input
                  type="email"
                  value={teacherSearch}
                ...
                {selectedTeachers.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-4">
                    {selectedTeachers.map(user => (
                      <div key={user.id} className="flex items-center gap-1.5 bg-border px-2 py-1 rounded-md">
                        <div className="w-4 h-4 bg-primary rounded-full flex items-center justify-center text-[10px] text-white">{user.name.charAt(0)}</div>
                        <span className="text-xs text-text-primary">{user.name.split(' ')[0]}</span>
                        <button type="button" onClick={() => setSelectedTeachers(selectedTeachers.filter(t => t.id !== user.id))} className="text-text-secondary hover:text-error ml-1">
                          <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                      </div>
                    ))}
